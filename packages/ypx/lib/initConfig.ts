...
import { defaultsDeep, camelCase, kebabCase } from 'lodash';
 ...
 */
export function normalizeConfig(userconfig: Record<string, any>)
{
	// 遍歷配置物件中的所有鍵名
 ...
 */
export function handleOptionsToConfig(argv: IYPXArguments, userconfig: Record<string, any> = {})
{
	// 建立配置的淺拷貝以避免修改原始物件
 ...
 */
export function buildConfig(argv: IYPXArguments, userconfig: Record<string, any> = {})
{
	// 合併預設配置、命令列選項與使用者配置，並正規化鍵名
	// Merge default config, CLI options, and user config, then normalize keys
	userconfig = normalizeConfig(defaultsDeep(handleOptionsToConfig(argv, userconfig), defaultConfig()));
